...
The `goks` CLI: `new`, `dev`, `build`, `ui`, `generate`, `page`.
...
`internal/lsp` should track `.gox` syntax changes made in `pkg/compiler` /
`internal/compiler` — keep them in sync in the same PR when GOX syntax
changes.
